...
for postureCount=1:6
    ...
    for roundCount=1:roundSize
        ...
        fullFolderName=[folderName 'RetrainingMix_' posture num2str(roundCount)];
            ...
            for trainingCount = 1 : 1
                ...
                for testingCount = 1 :numel(periodList)
                    ...
                    for userCount = 1:numel(userInvolved)
                        ...
                        %% ======= TRAINING !!! ======
                        % we do training every period, but in firs period positive is from 2+3
                        if (periodList(trainingCount)==periodList(testingCount))
                            ...
                            %                         trainPosData = [trainPosData;trainHistogram(1:40,:)];
                            trainPosData = [trainPosData;trainHistogram];
                            posHisTemp(userCount).posHist = trainPosData;
                            
                        else
                            posHisTemp(userCount).posHist = [posHisTemp(userCount).posHist; trainPosData];
                        end
                        
                        t = clock;
                        model = Training_pureSVM (FeatureIndex, posHisTemp(userCount).posHist, trainNegData, classifierNum,penaltyList,hiddenSizesNum);
