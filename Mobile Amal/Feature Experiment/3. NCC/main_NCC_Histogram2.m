...
coreFolderName = 'NCC';
rootFolderResults = 'Result final NCC Histogram2\5rounds100peoples';
                ...
                for testingCount = 1:numel(periodList)
                    ...
                    for userCount = 1:numel(userInvolved)
                        ...
                        if (periodList(trainingCount)==periodList(testingCount))
                            ...
                            t = clock;
                            [meanNeg{userCount}] = getMeanTrainingData(trainNegData,FeatureIndex);
                        ...
                        t2 = clock;
                        [meanData_new] = getMeanTrainingData(trainHistogram_new,FeatureIndex);
                        trainingTime2 = etime(clock,t2);
                        totalTIME_training{userCount,testingCount} = trainingTime1+trainingTime2;
                        
                        %% TESTING
                        t3 = clock;
                        [distanceTestWNewAll,distanceWithNegAll,distanceWithNegMultiAll,classresult,probability] = Nearest_Centroid_Classifier (FeatureIndex,testPosData,testNegData,meanData_new,meanNeg{userCount});
