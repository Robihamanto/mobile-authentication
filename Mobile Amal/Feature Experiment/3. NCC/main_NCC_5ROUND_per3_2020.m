...
%% PARAMETER SETTING
roundSize = 1;
folderRoot = '55NCCwNew-mixed';
folderNR = ['meeting0515\1rounds60peoples\' folderRoot '\'] ;
allAllavgeer=[];
numOfFlick = 5;
posture = 'mixed';
%remove user 5,6,13,31,32,33,34,37,38,45
userInvolved = [1:4 7:12 14:30 35 36 39:57 59:70];
% userInvolved =[39,5,18,23,26];
% userInvolved =[39,18,23,26];
% userInvolved = [14:30];
...
%FeatureIndex = [17:49];
FeatureIndex = [1:49];
%featureTable = [touchFeatureIndex];
%%
        ...
        for trainingCount = 1:1
            ...
            for testingCount = 1:6
                ...
                for userCount = 1:numel(userInvolved)
                    ...
                    %% LOAD NEGATIVE FOR TRAINING AND TESTING
%                     load([sampleset_path '\' posture '\user_' num2str(userID) '_period_' num2str(period(trainingCount)) '_round_' num2str(round) '_train_neg_sampleSet.mat'], 'trainNegHistogram');
%                     load([sampleset_path '\' posture '\user_' num2str(userID) '_period_' num2str(period(trainingCount)) '_round_' num2str(round) '_test_neg_sampleSet.mat'], 'testNegHistogram');
                    ...
                    trainHistogram_new=datasample(trainHistogram_new,5);
                    trainNegData = datasample(trainNegData,5);
                    trainPosData = datasample(trainPosData,5);
                    ...
                    if (period(trainingCount)==period(testingCount))
                        ...
                        trainHistogram=datasample(trainHistogram,5);
                        ...
                        trainNegDataTemp=datasample(trainNegDataTemp,5);
                        trainNegData = [trainNegData;trainNegDataTemp];
                        [meanNeg{userCount},meanOld{userCount},meanNegMulti{userCount}] = getMeanTrainingData(trainPosData,trainNegData,FeatureIndex);
                        
                    end
   
                    [meanData_new] = getMeanNewTrainingData(FeatureIndex, trainHistogram_new);
                    
                    %% TRANSFORMING AND TESTING
                    [distanceTestWNewAll,distanceWithNegAll,distanceWithNegMultiAll,classresult,probability] = Nearest_Centroid_Classifier (FeatureIndex,testPosData,testNegData,meanData_new,meanNeg{userCount},meanOld{userCount},meanNegMulti{userCount});
...
% % %                         xlswrite(excelName,distanceWithOldAll,'accuracy','B1');
% %                         xlswrite(excelName,distanceTestWNewAll,'accuracy','B1');
% %                         xlswrite(excelName,distanceWithNegAll,'accuracy','C1');
% %                         % xlswrite(excelName,distanceWithNegAll,'accuracy','B1');
% % %                         xlswrite(excelName,distanceWithNegMultiAll,'accuracy','D1');
%                     end
                ...
                filename = ['..\exp_result\' folderN '\TLaccu_Train' num2str(period(trainingCount)) '_test' num2str(period(testingCount))];
                excelName = [filename '.xlsx'];
