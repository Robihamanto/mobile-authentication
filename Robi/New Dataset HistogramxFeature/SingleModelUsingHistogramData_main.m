...
experimentMode = 'Single Model with histogram';
...
for postureString = postures
    ...
    for round = 1:roundSize
        ...
        negativeTrainingData = getHistogram(trainNegHistogram);
        negativeTestingData = getHistogram(testNegHistogram);
        ...
        for period = periods
            ...
            for userIndex = 1:numel(usersInvolved)
                ...
                positiveTrainingData = getHistogram(trainHistogram);
                positiveTestingData = getHistogram(testHistogram);
                
                if period == 3
                    load([sampleSetDataPath '\' posture '\' 'user_' num2str(user) '_period_' num2str(2) '_round_' num2str(round) '_train_sampleSet.mat']);
                    positiveTrainingData = [positiveTrainingData ;getHistogram(trainHistogram)];
